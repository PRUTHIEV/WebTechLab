...
public class MatrixAddition {
    ...
    public static void add(int [][]res,int [][]mat1,int [][]mat2,int row,int col){
        System.out.println(Thread.currentThread().getName() + " Perform the " + row + " th addition ");
        for(int i = 0 ; i < col ; i++){
            res[row][i] = mat1[row][i] + mat2[row][i];
        }
        System.out.println("The " + row + " row's sum is calculated using " + Thread.currentThread().getName());
    }
